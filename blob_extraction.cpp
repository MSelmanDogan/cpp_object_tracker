        ...
        for (size_t i = 0; i < contours.size(); i++) {
            ...
            if (blobArea >= minBlobArea && blobArea <= 10000) {
                ...
                for (size_t j = 0; j < previousMeasuredPosition.size(); j++) {
                    cv::circle(frame, previousMeasuredPosition[j], 5, cv::Scalar(255, 255, 0), -1);
                    cv::circle(frame, previousPredictedPosition[j], 5, cv::Scalar(0, 0, 255), -1);
                }
                std::cout << "if:" << std::endl;
            }
            else if (blobArea < minBlobArea && !previousPredictedPosition.empty() && !isObjectDetected) {
                std::cout << "previousPredictedPosition" << previousPredictedPosition.back() << std::endl;
                ...
                for (size_t j = 0; j < previousMeasuredPosition.size(); j++) {
                    cv::circle(frame, previousMeasuredPosition[j], 5, cv::Scalar(255, 0, 0), -1);
                    cv::circle(frame, previousPredictedPosition[j], 5, cv::Scalar(255, 255, 255), -1);
                }
                std::cout << "elif2" << std::endl;
            }
            else {
                std::cout << "else" << std::endl;
                // Bu kısıma başka bir işlem eklemeyi unutmayın veya bu else bloğunu kaldırın
            }
        }
